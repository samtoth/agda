...
-- TODO(sam): Chose a particular modal theory - with real 2-cells
-- to see how far one can get. This will require changing at least
-- the variable constructor of the Term data type.

-- | as an example, we consider the more or less easiest/natural non-posetal
-- theory. Generated by:
--   One mode (*), one generating 1-cell suc, and
--   a pair of generating 2 cells, src : Suc => id, tgt : Suc => id,
--   subject to no extra equations
--
-- We can naturally view the modalities as dimensions (natural numbers),
-- and terms of type <n| A > as n-dimensional cubes.

type Dim = Peano

-- | We have a convenient normal form for 2-cells:
...
-- | The obvious embedding Dim -> Cell gives us the identity 2-cells

idCell :: Dim -> Cell
idCell Zero = CZ
idCell (Succ d) = CS (idCell d)

-- | Composition of 2-cells. We assume they are apriory well typed

composeCell :: Cell -> Cell -> Cell
composeCell CZ CZ = CZ
composeCell (CS x) (CS y) = CS (composeCell x y)
composeCell x (Src y) = Src (composeCell x y)  -- (m => n) -> (So => m) -> (So => m)
composeCell x (Tgt y) = Tgt (composeCell x y)  -- (m => n) -> (So => m) -> (So => m)
composeCell _ _ = __IMPOSSIBLE__

cellWellTyped :: Cell -> Dim -> Dim -> Bool
cellWellTyped CZ Zero Zero = True
cellWellTyped (CS n) (Succ mu) (Succ nu) = cellWellTyped n mu nu
cellWellTyped (Src c) (Succ mu) nu = cellWellTyped c mu nu
cellWellTyped (Tgt c) (Succ mu) nu = cellWellTyped c mu nu
cellWellTyped _ _ _ = False

data MTTModality = MTTMod
  { _mod  :: Dim
  , _lock :: Dim
  } deriving (Show, Ord, Eq)

instance NFData Dim where
  rnf Zero = ()
  rnf (Succ n) = rnf n

instance NFData MTTModality where
  rnf (MTTMod a b) = seq (rnf a) $ rnf b


instance HasRange MTTModality where
  getRange _ = noRange

instance SetRange MTTModality where
  setRange _ = id

instance KillRange MTTModality where
  killRange rel = rel -- no range to kill

isIdCell :: Cell -> Bool
isIdCell CZ = True
isIdCell (CS n) = isIdCell n
isIdCell _ = False

instance Pretty Cell where
  pretty c | isIdCell c = mempty
  pretty CZ = mempty
  pretty (CS n) = "S" <> pretty n
  pretty (Src n) = "0" <> pretty n
  pretty (Tgt n) = "1" <> pretty n

cellFromString :: String -> Maybe Cell
cellFromString [] = Just CZ
cellFromString ('Z':rs) = Just CZ
cellFromString ('S':rs) = CS <$> cellFromString rs
cellFromString ('0':rs) = Src <$> cellFromString rs
cellFromString ('1':rs) = Tgt <$> cellFromString rs
cellFromString _ = Nothing

instance Pretty Dim where
  pretty = pretty . toInteger

instance Pretty MTTModality where
  pretty (MTTMod Zero Zero) = mempty
  pretty (MTTMod m Zero) = "@" <> pretty m
  pretty (MTTMod Zero l) = "@0/" <> pretty l
  pretty (MTTMod m l) = "@" <> pretty m <> "/" <> pretty l

-- | A lens to access the 'Cohesion' attribute in data structures.
--   Minimal implementation: @getMTT@ and @mapMTT@.
class LensMTT a where
  getMTT :: a -> MTTModality

  setMTT :: MTTModality -> a -> a
  setMTT h = mapMTT (const h)

  mapMTT :: (MTTModality -> MTTModality) -> a -> a


instance LensMTT MTTModality where
  getMTT = id
  setMTT = const
  mapMTT = id


instance Null MTTModality where
  empty = MTTMod 0 0
  null = \case
    (MTTMod Zero Zero) -> True
    _ -> False
...
instance HasRange ArgInfo where
  getRange (ArgInfo h m mm o _fv a) = getRange (h, m, mm, o, a)

instance KillRange ArgInfo where
  killRange (ArgInfo h m mm o fv a) = killRangeN ArgInfo h m mm o fv a
...
instance NFData ArgInfo where
  rnf (ArgInfo a b c d e f) = rnf a `seq` rnf b `seq` rnf c `seq` rnf d `seq` rnf e `seq` rnf f
...
instance LensMTT ArgInfo where
  getMTT = argInfoMTT
  mapMTT f ai = ai { argInfoMTT = f (argInfoMTT ai)}

instance Null ArgInfo where
  empty = defaultArgInfo
  null (ArgInfo h m mm _o _fv ann) = and [ null h, null m, null ann, null mm ]
